...
export default factories.createCoreService(
	...
	({ strapi }) => ({
		async getDailyRecommended() {
			...
			if (selected?.length > 0) {
				existing
					.filter(
						(recipe) =>
							!selected.some((newRecipe) => newRecipe.id === recipe.id)
					)
		...
		async deleteUnlinkedMedia() {
			...
			const allMedia = await strapi.entityService.findMany(
				"plugin::upload.file",
				{
					fields: ["id", "url"],
					limit: -1,
      ...
      categories?.forEach((category) => {
        if ((category as any).icon?.id) {
          linkedMediaIdsSet.add((category as any).icon?.id);
        }
      });

			// 3. Filter out unlinked files
			const unlinkedFiles = allMedia.filter(
				(file) => !linkedMediaIdsSet.has((file as any).id)
			);
			...
			strapi.log.info(
				`Cleanup done. Deleted ${unlinkedFiles.length} unlinked files.`
			);
		},
	})
);
